...
/**
  * @brief  Uart buffer for received message
  */
...
int main(void)
{
    Clk_Update(); //Update MCU clock 
    Boot_Init(); //Init 
    
    Transmit_ACK(&huart); //sends acknowledge byte

    /*If ACK is not received, then pass loop*/
	...
	for(;;)
	{
        /*wait for timeout_value to receive message*/
...
/**
  * @brief  Initialize boot by setting pin and uart configuration.
  * @retval None
  */
static void Boot_Init(void)
{
...
/**
  ...
  * @param  handle pointer to a UART_HandleTypeDef structure that contains
...
/**
  * @brief  Sends no-acknowledge byte
  * @param  handle pointer to a UART_HandleTypeDef structure that contains
  *         the configuration information for the specified UART.
  * @retval None
  */
...
/**
  * @brief  Check buffer and length 
  * @param  pBuffer pointer to a uint8_t to hold buffer array
  * @param  len variable to uint32_t to hold buffer length
  * @retval if result 0
            -1
            else 
            -0
